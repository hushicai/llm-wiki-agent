...
规则：
- **文件名**：以源文件名作为文件名，保留原始语言形态，不转写、不翻译、不加括号备注、不加版本号或产品名后缀。
...
输出：创建 `raw/2025-05-01-Understanding Transformers.md`：
```markdown
...
- **新概念** → 在 wiki/ 中创建新页面。文件名以核心概念名称作为文件名，保留原始语言形态，不转写、不翻译、不加括号备注、不加版本号或产品名后缀。
...
- **不要中途输出总结或状态报告。** 在完成所有工作之前，只通过工具调用进行操作。
- **坚持到底。** 抓取→编译→级联更新→index→log，全部完成才输出最终总结。
- **不要跳过步骤。** 如果遇到问题，尝试其他方法解决，不要提前退出。
